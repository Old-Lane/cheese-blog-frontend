...
import { followUserApi, getUserFansApi, unfollowUserApi } from '@/api/follow';
import { useEmitt } from '@/hook/web/useEmitt';
import Empty from '@/views/error/empty.vue';

const { currentRoute, push } = useRouter()

const fans = ref<Array<UserCardType>>([])
...
const getData = () => {
  getUserFansApi(currentRoute.value.params.uid).then(res => {
    fans.value = res.data
  })
}
...
<template>
  <n-card>
    <div v-if="fans.length === 0" class="mb-10">
      <Empty messege="还没有粉丝噢" />
    </div>
    <div v-else v-for="item in fans">
      <div class="flex items-center justify-between h-30">
        ...
        <div>
          <n-button size="small" v-if="!item.isFollowed" @click="followUser(item)" ghost type="warning"
            class="mr-5 w-30">＋关注</n-button>
